...
std::string RpcRequestHandler::handleRequest( const char* src, std::size_t srcsize)
{
	...
	switch( (AnalyzerObjectBuilderConst::MethodId)methodId)
	{
		...
		case AnalyzerObjectBuilderConst::Method_createPosTaggerInstance:
		{
			RpcSerializer msg;
			...
			unsigned char classId_0; unsigned int objId_0;
			serializedMsg.unpackObject( classId_0, objId_0);
			p0 = obj->createPosTaggerInstance(p1,p2);
			const char* err = m_errorhnd->fetchError();
			if (err)
			{
				msg.packByte( MsgTypeError);
	...
	switch( (PosTaggerInstanceConst::MethodId)methodId)
	{
		...
		case PosTaggerInstanceConst::Method_markupDocument:
		{
			RpcSerializer msg;
			std::string p0;
			...
			p3 = serializedMsg.unpackDocumentClass();
			p4 = serializedMsg.unpackString();
			p0 = obj->markupDocument(p1,p2,p3,p4);
			const char* err = m_errorhnd->fetchError();
			if (err)
			{
				msg.packByte( MsgTypeError);
	...
	switch( (QueryEvalConst::MethodId)methodId)
	{
		...
		case QueryEvalConst::Method_getWeightingFeatureSets:
		{
			RpcSerializer msg;
			std::vector<std::string> p0;
			p0 = obj->getWeightingFeatureSets();
			const char* err = m_errorhnd->fetchError();
			if (err)
			{
				msg.packByte( MsgTypeError);
	...
	switch( (QueryProcessorConst::MethodId)methodId)
	{
		...
		case QueryProcessorConst::Method_getResourceFilePath:
		{
			RpcSerializer msg;
			std::string p0;
			std::string p1;
	...
	switch( (SentenceLexerInstanceConst::MethodId)methodId)
	{
		...
		case SentenceLexerInstanceConst::Method_call:
		{
			RpcSerializer msg;
	...
	switch( (StatisticsBuilderConst::MethodId)methodId)
	{
		...
		case StatisticsBuilderConst::Method_getMessages:
		{
			RpcSerializer msg;
			...
			msg.packSize( p0.size());
			for (std::size_t ii=0; ii < p0.size(); ++ii) {
				msg.packStatisticsMessage( p0[ii]);
			}
			msg.packCrc32();
			return msg.content();
		}
		case StatisticsBuilderConst::Method_commit:
		{
			RpcSerializer msg;
			bool p0;
			p0 = obj->commit();
	...
	switch( (StatisticsProcessorConst::MethodId)methodId)
	{
		...
		case StatisticsProcessorConst::Method_getUpperBoundTimeStamp:
		{
			RpcSerializer msg;
			TimeStamp p0;
			std::string p1;
			...
			p0 = obj->getUpperBoundTimeStamp(p1,p2);
			const char* err = m_errorhnd->fetchError();
			if (err)
			{
				msg.packByte( MsgTypeError);
			...
			msg.packByte( MsgTypeAnswer);
			msg.packTimeStamp( p0);
			msg.packCrc32();
			return msg.content();
		}
		case StatisticsProcessorConst::Method_loadChangeMessage:
		{
			RpcSerializer msg;
			...
			p1 = serializedMsg.unpackString();
			p2 = serializedMsg.unpackTimeStamp();
			p0 = obj->loadChangeMessage(p1,p2);
			const char* err = m_errorhnd->fetchError();
			if (err)
			{
				msg.packByte( MsgTypeError);
		...
		case StatisticsProcessorConst::Method_releaseStatistics:
		{
			RpcSerializer msg;
			std::string p1;
			TimeStamp p2;
	...
	case ClassId_StatisticsStorageClient:
	{
	StatisticsStorageClientInterface* obj = getObject<StatisticsStorageClientInterface>( classId, objId);
	switch( (StatisticsStorageClientConst::MethodId)methodId)
	{
		case StatisticsStorageClientConst::Method_Destructor:
		{
			deleteObject( classId, objId);
			return std::string();
		}
		case StatisticsStorageClientConst::Method_reload:
		{
			RpcSerializer msg;
			bool p0;
			std::string p1;
		...
		case StatisticsStorageClientConst::Method_diskUsage:
		{
			RpcSerializer msg;
			long p0;
			p0 = obj->diskUsage();
		...
		case StatisticsStorageClientConst::Method_getConfigParameters:
		{
			RpcSerializer msg;
			const char** p0;
			p0 = obj->getConfigParameters();
		...
		case StatisticsStorageClientConst::Method_config:
		{
			RpcSerializer msg;
			std::string p0;
			p0 = obj->config();
		...
		case StatisticsStorageClientConst::Method_nofDocuments:
		{
			RpcSerializer msg;
			GlobalCounter p0;
			p0 = obj->nofDocuments();
		...
		case StatisticsStorageClientConst::Method_documentFrequency:
		{
			RpcSerializer msg;
			GlobalCounter p0;
			std::string p1;
		...
		case StatisticsStorageClientConst::Method_storageTimeStamp:
		{
			RpcSerializer msg;
			TimeStamp p0;
			std::string p1;
			p1 = serializedMsg.unpackString();
			p0 = obj->storageTimeStamp(p1);
			const char* err = m_errorhnd->fetchError();
			if (err)
			{
				msg.packByte( MsgTypeError);
		...
		case StatisticsStorageClientConst::Method_putStatisticsMessage:
		{
			RpcSerializer msg;
			bool p0;
			StatisticsMessage p1;
			std::string p2;
			p1 = serializedMsg.unpackStatisticsMessage();
			p2 = serializedMsg.unpackString();
			p0 = obj->putStatisticsMessage(p1,p2);
			const char* err = m_errorhnd->fetchError();
			if (err)
			{
				msg.packByte( MsgTypeError);
		...
		case StatisticsStorageClientConst::Method_getStatisticsProcessor:
		{
			RpcSerializer msg;
		...
		case StatisticsStorageClientConst::Method_close:
		{
			RpcSerializer msg;
			obj->close();
			const char* err = m_errorhnd->fetchError();
		...
		case StatisticsStorageClientConst::Method_compaction:
		{
			RpcSerializer msg;
			obj->compaction();
			const char* err = m_errorhnd->fetchError();
	...
	case ClassId_StatisticsStorage:
	{
	StatisticsStorageInterface* obj = getObject<StatisticsStorageInterface>( classId, objId);
	switch( (StatisticsStorageConst::MethodId)methodId)
	{
		case StatisticsStorageConst::Method_Destructor:
		{
			deleteObject( classId, objId);
			return std::string();
		}
		case StatisticsStorageConst::Method_createClient:
		{
			RpcSerializer msg;
			StatisticsStorageClientInterface* p0;
			std::string p1;
		...
		case StatisticsStorageConst::Method_createStorage:
		{
			RpcSerializer msg;
			bool p0;
			std::string p1;
		...
		case StatisticsStorageConst::Method_getConfigDescription:
		{
			RpcSerializer msg;
			const char* p0;
			p0 = obj->getConfigDescription();
		...
		case StatisticsStorageConst::Method_getConfigParameters:
		{
			RpcSerializer msg;
			const char** p0;
			p0 = obj->getConfigParameters();
	...
	switch( (StorageClientConst::MethodId)methodId)
	{
		...
		case StorageClientConst::Method_reload:
		{
			RpcSerializer msg;
			bool p0;
			std::string p1;
		...
		case StorageClientConst::Method_blockStatistics:
		{
			RpcSerializer msg;
			BlockStatistics p0;
			p0 = obj->blockStatistics();
			const char* err = m_errorhnd->fetchError();
			if (err)
			{
				msg.packByte( MsgTypeError);
		...
		case StorageClientConst::Method_structTypeNumber:
		{
			RpcSerializer msg;
			Index p0;
			std::string p1;
			p1 = serializedMsg.unpackString();
			p0 = obj->structTypeNumber(p1);
			const char* err = m_errorhnd->fetchError();
			if (err)
			{
				msg.packByte( MsgTypeError);
		...
		case StorageClientConst::Method_createStructTypeIterator:
		{
			RpcSerializer msg;
			ValueIteratorInterface* p0;
			unsigned char classId_0; unsigned int objId_0;
			serializedMsg.unpackObject( classId_0, objId_0);
			p0 = obj->createStructTypeIterator();
			const char* err = m_errorhnd->fetchError();
			if (err)
			{
				msg.packByte( MsgTypeError);
		...
		case StorageClientConst::Method_getNextChangeStatisticsTimeStamp:
		{
			RpcSerializer msg;
			TimeStamp p0;
			TimeStamp p1;
			p1 = serializedMsg.unpackTimeStamp();
			p0 = obj->getNextChangeStatisticsTimeStamp(p1);
			const char* err = m_errorhnd->fetchError();
			if (err)
			{
				msg.packByte( MsgTypeError);
		...
		case StorageClientConst::Method_loadInitStatisticsMessages:
		{
			RpcSerializer msg;
			std::vector<StatisticsMessage> p0;
			p0 = obj->loadInitStatisticsMessages();
			const char* err = m_errorhnd->fetchError();
			if (err)
			{
				msg.packByte( MsgTypeError);
		...
		case StorageClientConst::Method_getConfigParameters:
		{
			RpcSerializer msg;
			const char** p0;
			p0 = obj->getConfigParameters();
		...
		case StorageClientConst::Method_compaction:
		{
			RpcSerializer msg;
			obj->compaction();
			const char* err = m_errorhnd->fetchError();
	...
	switch( (StorageDocumentConst::MethodId)methodId)
	{
		...
		case StorageDocumentConst::Method_addSearchIndexStructure:
		{
			RpcSerializer msg;
			std::string p1;
			IndexRange p2;
			IndexRange p3;
			p1 = serializedMsg.unpackString();
			p2 = serializedMsg.unpackIndexRange();
			p3 = serializedMsg.unpackIndexRange();
	...
	switch( (StorageDocumentUpdateConst::MethodId)methodId)
	{
		...
		case StorageDocumentUpdateConst::Method_addSearchIndexStructure:
		{
			RpcSerializer msg;
			std::string p1;
			IndexRange p2;
		...
		case StorageDocumentUpdateConst::Method_clearSearchIndexStructures:
		{
			RpcSerializer msg;
	...
	switch( (StorageConst::MethodId)methodId)
	{
		...
		case StorageConst::Method_destroyStorage:
		{
			RpcSerializer msg;
			bool p0;
			std::string p1;
	...
	switch( (SummarizerFunctionInstanceConst::MethodId)methodId)
	{
		...
		case SummarizerFunctionInstanceConst::Method_doPopulate:
		{
			RpcSerializer msg;
			bool p0;
			p0 = obj->doPopulate();
	...
	switch( (TextProcessorConst::MethodId)methodId)
	{
		...
		case TextProcessorConst::Method_createPosTaggerData:
		{
			RpcSerializer msg;
			PosTaggerDataInterface* p0;
			TokenizerFunctionInstanceInterface* p1;
			unsigned char classId_1; unsigned int objId_1;
			serializedMsg.unpackObject( classId_1, objId_1);
			if (classId_1 != ClassId_TokenizerFunctionInstance) throw strus::runtime_error( "%s", _TXT("error in RPC serialzed message: output parameter object type mismatch"));
			p1 = getObject<TokenizerFunctionInstanceInterface>( classId_1, objId_1);
			unsigned char classId_0; unsigned int objId_0;
			serializedMsg.unpackObject( classId_0, objId_0);
			p0 = obj->createPosTaggerData(p1);
			const char* err = m_errorhnd->fetchError();
			if (err)
			{
				msg.packByte( MsgTypeError);
		...
		case TextProcessorConst::Method_getPosTagger:
		{
			RpcSerializer msg;
			const PosTaggerInterface* p0;
			unsigned char classId_0; unsigned int objId_0;
			serializedMsg.unpackObject( classId_0, objId_0);
			p0 = obj->getPosTagger();
			const char* err = m_errorhnd->fetchError();
		...
		case TextProcessorConst::Method_createTokenMarkupInstance:
		{
			RpcSerializer msg;
			TokenMarkupInstanceInterface* p0;
			unsigned char classId_0; unsigned int objId_0;
			serializedMsg.unpackObject( classId_0, objId_0);
			p0 = obj->createTokenMarkupInstance();
			const char* err = m_errorhnd->fetchError();
			if (err)
			{
				msg.packByte( MsgTypeError);
	...
	switch( (VectorStorageClientConst::MethodId)methodId)
	{
		...
		case VectorStorageClientConst::Method_createFeatureValueIterator:
		{
			RpcSerializer msg;
			ValueIteratorInterface* p0;
			unsigned char classId_0; unsigned int objId_0;
			serializedMsg.unpackObject( classId_0, objId_0);
			p0 = obj->createFeatureValueIterator();
			const char* err = m_errorhnd->fetchError();
			if (err)
			{
				msg.packByte( MsgTypeError);
		...
		case VectorStorageClientConst::Method_normalize:
		{
			RpcSerializer msg;
			WordVector p0;
			WordVector p1;
		...
		case VectorStorageClientConst::Method_compaction:
		{
			RpcSerializer msg;
			obj->compaction();
			const char* err = m_errorhnd->fetchError();
	...
	switch( (VectorStorageTransactionConst::MethodId)methodId)
	{
		...
		case VectorStorageTransactionConst::Method_defineVector:
		{
			RpcSerializer msg;
			std::string p1;
			std::string p2;
			...
			p1 = serializedMsg.unpackString();
			p2 = serializedMsg.unpackString();
			p3 = serializedMsg.unpackWordVector();
		...
		case VectorStorageTransactionConst::Method_defineFeatureType:
		{
			RpcSerializer msg;
			std::string p1;
			p1 = serializedMsg.unpackString();
		...
		case VectorStorageTransactionConst::Method_defineFeature:
		{
			RpcSerializer msg;
			std::string p1;
			std::string p2;
		...
		case VectorStorageTransactionConst::Method_clear:
		{
			RpcSerializer msg;
			obj->clear();
			const char* err = m_errorhnd->fetchError();
